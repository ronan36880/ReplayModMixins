package com.moonsworth.lunar.replaymod.v1_8.link;
...
public class ReplayModGuiLink_v1_8 extends EventRegistrations implements ReplayModLink {
    ...
    public ReplayModGuiLink_v1_8() {
        ...
        EventBus.getBus().register(EventScrollSwapItem.class, e -> {
            if (this.isViewingReplay()) {
                InputReplayTimer.handleScroll((int) e.getDirection());
                e.setCancelled(true);
            }
        });
    ...
    public boolean shouldRenderBrand(GuiScreenBridge guiScreenBridge) {
        if (ReplayModGuiLink_v1_8.hasInputOverlayOpen) {
            return false;
        }
